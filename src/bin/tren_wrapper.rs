    ...
    loop {
        ...
        match socket.recv_from(&mut buf) {
            Ok((n, src)) => {
                ...
                let reply = handle_request(&msg, &workdir, &tree, &subs, src);
                if let Some(r) = reply {
...
fn handle_request(
    ...
    if head == "SUBMIT" {
        ...
        match allocate_node(workdir, tree, subs, cmd, deps) {
...
fn allocate_node(
    ...
    // then exec the command. Recursive child-thread spawning of the binary
    // tree happens implicitly when the user issues further SUBMITs that
    // become this node's descendants.
    let workdir_t = workdir.to_path_buf();
    let tree_t    = Arc::clone(tree);
    let subs_t    = Arc::clone(subs);
    let addr_t    = addr.clone();
    thread::spawn(move || run_node(workdir_t, tree_t, subs_t, addr_t));

    Ok(addr)
}

fn run_node(workdir: PathBuf, tree: Tree, subs: SubMap, addr: String) {
    // ── 1. Wait for deps ───────────────────────────────────────────────────
    let deps = {
        ...
        Ok(mut child) => {
            ...
            let _ = fs::write(node_dir(&workdir, &addr).join("pid"), pid.to_string());
            {
                ...
                if let Some(r) = g.get_mut(&addr) {
                    ...
                    // child must be moved out of `child` after wait, keep the
                    // handle in record for KILL.
                }
            }
            ...
            update_record_state(&tree, &addr, s.clone());
            // Remove the child handle now that it is reaped.
            {
                let mut g = tree.lock().unwrap();
